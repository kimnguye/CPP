...
void ClapTrap::attack(const std::string& target)
{
    if (_energyPoints > 0 && _hitPoints > 0)
    {
        ...
        std::cout << "ClapTrap " << _name << " can't attack! No energy or hit points left.\n";
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (_energyPoints > 0)
    {
		...
		{
			std::cout << "ClapTrap " << _name << " repairs itself and recovers all his hit points! Current hit points: " << _hitPoints << std::endl;
			_hitPoints = hp_max;
		}
		else
...
void	ClapTrap::print()
{
	std::cout << CYAN << "ClapTrap " << _name << ":"<< RST << std::endl;
